...
test("Contact Form - JSON", async ({ page, locators }) => {
  ...
  await expect(locators.firstNameInput).toHaveValue(
    testDataJson.user1.firstName,
  );
...
test("Contact Form - TypeScript", async ({ page, locators }) => {
  ...
  await expect(locators.firstNameInput).toHaveValue(
    loginCredentials.user1.firstName,
  );

  await locators.lastNameInput.fill(loginCredentials.user1.lastName);
  await expect(locators.lastNameInput).toHaveValue(
    loginCredentials.user1.lastName,
  );
  ...
  await expect(locators.messageInput).toHaveValue(
...
test("Registration - JSON", async ({ page, locators }) => {
  test.skip(
    !!process.env.CI,
    "Skipping on CI - security verification required",
  );
  ...
  await expect(page.locator('[data-test="first-name"]')).toHaveValue(
    testDataJson.user1.firstName,
  );
  ...
  await expect(page.locator('[data-test="last-name"]')).toHaveValue(
    testDataJson.user1.lastName,
  );

  await page.locator('[data-test="dob"]').fill(testDataJson.user1.dob);
  await expect(page.locator('[data-test="dob"]')).toHaveValue(
    testDataJson.user1.dob,
  );
  ...
  await expect(page.locator('[data-test="postal_code"]')).toHaveValue(
    testDataJson.user1.postalCode,
  );

  await page
    .locator('[data-test="house_number"]')
    .fill(testDataJson.user1.houseNo);
  await expect(page.locator('[data-test="house_number"]')).toHaveValue(
    testDataJson.user1.houseNo,
  );

  await page.locator('[data-test="phone"]').fill(testDataJson.user1.phone);
  await expect(page.locator('[data-test="phone"]')).toHaveValue(
    testDataJson.user1.phone,
  );
  ...
  await page
    ...
    .pressSequentially(testDataJson.user1.password, { delay: 100 });
  await expect(page.locator('[data-test="password"]')).toHaveValue(
    testDataJson.user1.password,
  );
...
test("Registration - TypeScript Data", async ({ page, locators }) => {
  ...
  await expect(page.locator('[data-test="postal_code"]')).toHaveValue(
    loginCredentials.user1.postalCode,
  );
  ...
  await page
    .locator('[data-test="password"]')
    .pressSequentially(loginCredentials.user1.password, { delay: 100 });
  await expect(page.locator('[data-test="password"]')).toHaveValue(
    loginCredentials.user1.password,
  );
  await expect(page.locator('[data-test="register-submit"]')).toBeEnabled({
    timeout: 15000,
  });
...
test("Delete item from cart", async ({ page, locators }) => {
  ...
  await expect(page.locator("aw-wizard")).toContainText(
